...
public class LoginPageScenerio {
    ...
    @Mając("^Załadowaną stronę logowana$")
    public void ZaladowanaStronaLogowania() {
        driver.get("http://automationpractice.com/index.php?controller=authentication&back=my-account");
    }

    @Kiedy("^Wprowadzam poprawnego użytkownika \"([^\"]*)\" i hasło \"([^\"]*)\"$")
    ...
    @I("^Kliknełem przycisk logowania by się zalogować$")
    public void KliknieciePrzyciskuLogowania() {
        LoginPage login = new LoginPage(driver);
        login.loginButtonClick();
    }

    @Wtedy("^Ukazuje mi się strona logowania$")
